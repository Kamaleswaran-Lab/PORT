...
#SBATCH --output=/path/to/CHD_MEDS/results_v4/slurm_occlusion_pp_%j.log
#SBATCH --error=/path/to/CHD_MEDS/results_v4/slurm_occlusion_pp_%j.err
...
echo "Per-patient occlusion (saves per-encounter predictions per category)"
...
OUT = Path("/path/to/CHD_MEDS/results_v4/evaluation/per_patient_occlusion")
...
def evaluate_with_predictions(category=None):
    """Return per-encounter predictions (sid, pt_us, y_true, y_prob)."""
    sids, pts, yt, yp = [], [], [], []
...
# ── Baseline (no mask) ──
log.info("Baseline evaluation (per-encounter)...")
df_base = evaluate_with_predictions()
df_base.to_parquet(OUT / "predictions_baseline.parquet", index=False)
b_auroc = roc_auc_score(df_base.y_true, df_base.y_prob)
b_auprc = average_precision_score(df_base.y_true, df_base.y_prob)
log.info(f"Baseline n={len(df_base):,}  AUROC={b_auroc:.4f}  AUPRC={b_auprc:.4f}")

# ── Per-category masking ──
summary = [{"Category": "Baseline", "Tokens": "-", "AUROC": b_auroc, "AUPRC": b_auprc,
...
for cat in CATEGORIES:
    ...
    summary.append({"Category": cat, "Tokens": n_tok, "AUROC": a, "AUPRC": p,
                    "Delta_AUROC": a - b_auroc, "Delta_AUPRC": p - b_auprc})
    log.info(f"  {cat}: AUROC={a:.4f} (Δ{a-b_auroc:+.4f})  AUPRC={p:.4f} (Δ{p-b_auprc:+.4f})")

pd.DataFrame(summary).sort_values("Delta_AUROC").to_csv(
    OUT / "summary.csv", index=False)
log.info(f"\nSaved {len(summary)} predictions parquets + summary.csv to {OUT}")
PYEOF

echo "Per-patient occlusion complete"
